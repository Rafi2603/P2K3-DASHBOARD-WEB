...
router.post('/addstruktur', async (req, res) => {
   ...
   if (!nama || !jabatan || !posisi) {
      return res.status(400).json({
         message: 'Nama, jabatan, dan posisi wajib diisi'
      });
   }
...
router.delete('/deletestruktur', async (req, res) => {
   ...
   if (!struktur_id) {
      return res.status(400).json({
         message: 'ID struktur wajib diisi'
      });
   }
...
router.delete('/delete-multiple-struktur', async (req, res) => {
   ...
   if (!Array.isArray(ids) || ids.length === 0) {
      return res.status(400).json({
         message: "Tidak ada ID yang diterima untuk dihapus."
      });
   }
...
router.post("/addrekap", async (req, res) => {
   ...
   try {
      ...
      res.status(201).json({
         message: "Data berhasil ditambahkan dengan perhitungan otomatis"
      });
...
router.put("/updaterekap", async (req, res) => {
   ...
   try {
      ...
      res.status(200).json({
         message: "Data berhasil diperbarui dengan perhitungan otomatis"
      });
   } catch (error) {
...
router.delete("/deleterekap", async (req, res) => {
   ...
   if (!rekapdata_id) {
      return res.status(400).json({
         message: "ID Rekap Data wajib diisi"
      });
   }
...
router.post('/addkecelakaankerja', async (req, res) => {
   ...
   if (!tanggal || !nama || !kategori_kecelakaan) {
      return res.status(400).json({
         message: 'Tanggal, Nama, dan Kategori Kecelakaan wajib diisi'
      });
   }
...
router.put('/updatekecelakaan', async (req, res) => {
   ...
   try {
      ...
      res.status(200).json({
         message: 'Data kecelakaan kerja berhasil diperbarui',
         data: result.rows[0]
      });
   } catch (error) {
      console.error('Error updating data:', error);
...
router.delete('/deletekecelakaan', async (req, res) => {
   ...
   if (!kecelakaankerja_id) {
      return res.status(400).json({
         message: 'ID Kecelakaan Kerja wajib diisi'
      });
   }
   ...
   try {
      ...
      res.status(200).json({
         message: 'Data kecelakaan kerja berhasil dihapus',
         data: result.rows[0]
      });
   } catch (error) {
      console.error('Error deleting data:', error);
...
router.delete('/deletekejadian', async (req, res) => {
   ...
   if (!kejadian_id) {
      return res.status(400).json({
         message: 'ID kejadian wajib diisi'
      });
   }
...
router.post('/addchecklist', upload.fields([{
   ...
   if (!section || !indikator_k3 || !expired_date) {
      return res.status(400).json({
         message: 'Field section, indikator_k3, dan expired_date wajib diisi.'
      });
   }
   ...
   try {
      ...
      res.status(201).json({
         message: 'Data checklist K3 berhasil ditambahkan.'
      });
   } catch (error) {
...
router.delete('/deletechecklist', async (req, res) => {
   ...
   if (!checklist_id) {
      return res.status(400).json({
         message: 'ID checklist wajib diisi'
      });
   }
